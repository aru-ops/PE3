...
# Handles menus, game states, spawning, collisions, UI, and settings (including car color)
# Added car color selection button in settings
...
def reset_game():
    ...
    powerups.empty()
    coins.empty()
    player = Player(settings["car_color"])   # create new player car with chosen color
...
# Settings screen buttons: difficulty and car color
btn_easy = Button(200, 120, 200, 40, "Easy")
btn_normal = Button(200, 170, 200, 40, "Normal")
btn_hard = Button(200, 220, 200, 40, "Hard")
# Button for cycling car color (displays current color)
btn_color = Button(200, 280, 200, 40, f"Car Color: {settings['car_color']}", (100,100,200), (150,150,255))
...
while running:
    ...
    for event in events:
        ...
        # ----- Settings state -----
            ...
            if btn_hard.is_clicked(event):
                ...
                save_settings(settings)
            # Cycle car color: red -> blue -> green -> red
            if btn_color.is_clicked(event):
                ...
                save_settings(settings)               # persist to JSON
                btn_color.text = f"Car Color: {settings['car_color']}"   # update button label
    ...
    if state == "MENU":
        ...
        curr_diff = font.render(f"Current Difficulty: {settings['difficulty'].upper()}", True, (0, 255, 0))
        screen.blit(curr_diff, (WIDTH//2 - curr_diff.get_width()//2, 370))
        btn_easy.draw(screen)
        btn_normal.draw(screen)
        btn_hard.draw(screen)
        btn_color.draw(screen)               # draw car color button
        ...
        score += 0.2 if not player.nitro_active else 0.5   # score faster with nitro
